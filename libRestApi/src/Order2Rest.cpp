...
int COrder2Rest::getHistoricalData(const char* symbol, const char* period, time_t start, time_t end, bool maxRange, TblCandle** pTblCandle[])
{
	...
	while (cur <= end) {
		...
		if (getHistoricalData(symbol, period, cur, to, tmpCandleList) < 0) {
			return RET_FAILED;
		}
...
int COrder2Rest::getHistoricalData(const char* symbol, const char* period, time_t start, time_t end, vector<TblCandle*>& tblCandleList)
{
	...
	params.push_back(ReqParam{"$start", transfTime(start) });
	params.push_back(ReqParam{"$end", transfTime(end) });
	curlObj->setEasyPerform(&params);
